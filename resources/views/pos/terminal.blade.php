    ...
    <script>
        ...
        function generateBarcode(text) {
            ...
            var patterns = [
                ...
                '11110110110','11110101000','11110100010'
            ];
            var checksum = 104;
            var result = patterns[104];
            for (var i = 0; i < text.length; i++) {
                var val = code128b[text[i]];
            ...
            for (var j = 0; j < result.length; j++) {
                if (result[j] === '1') {
                    svg += '<rect x="' + x + '" y="0" width="' + barWidth + '" height="40" fill="#000"/>';
                }
                x += barWidth;
            }
            svg += '<text x="100" y="48" text-anchor="middle" font-family="Cairo" font-size="10" font-weight="700">' + text + '</text>';
            svg += '</svg>';
            return svg;
        }
        ...
        async function printInvoice() {
            ...
            for (var i = 0; i < cart.length; i++) {
                ...
                var qtyDisplay = item.isWeight ? item.qty.toFixed(3) + ' كجم' : item.qty;
                itemsHtml += '<tr>';
                itemsHtml += '<td>' + item.name + '</td>';
                itemsHtml += '<td class="center">' + qtyDisplay + '</td>';
                itemsHtml += '<td class="center">' + item.price.toFixed(3) + '</td>';
                itemsHtml += '<td class="left">' + itemTotal.toFixed(3) + '</td>';
                itemsHtml += '</tr>';
            }
            ...
            var barcodeSvg = generateBarcode(orderNumber);
